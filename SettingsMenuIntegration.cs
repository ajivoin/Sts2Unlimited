...
public static class SettingsMenuIntegration
{
    ...
    public static void InitializeSettingsMenuUI(Harmony harmony)
    {
        try
        {
            ...
            if (screenType == null) { Log.LogMessage(LogLevel.Warn, LogType.Generic, "[Sts2Unlimited] NSettingsScreen type not found."); return; }

            var readyMethod = screenType.GetMethod("_Ready",
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            if (readyMethod == null) { Log.LogMessage(LogLevel.Warn, LogType.Generic, "[Sts2Unlimited] NSettingsScreen._Ready not found."); return; }
            ...
            Log.LogMessage(LogLevel.Info, LogType.Generic, "[Sts2Unlimited] Patched NSettingsScreen._Ready.");
        }
        catch (Exception e) { Log.LogMessage(LogLevel.Error, LogType.Generic, $"[Sts2Unlimited] Patch failed: {e.Message}"); }
    }
    ...
    private static void Patch_NSettingsScreen_Ready(object __instance)
    {
        ...
        try
        {
            ...
            if (masterType == null) { Log.LogMessage(LogLevel.Warn, LogType.Generic, "[Sts2Unlimited] NMasterVolumeSlider not found."); return; }
            ...
            if (template == null) { Log.LogMessage(LogLevel.Warn, LogType.Generic, "[Sts2Unlimited] NMasterVolumeSlider node not found."); return; }
            ...
            {
                Log.LogMessage(LogLevel.Warn, LogType.Generic, "[Sts2Unlimited] %ModdingDivider not found — cannot place slider.");
                return;
            }
            ...
            tree.Connect(SceneTree.SignalName.ProcessFrame, Callable.From(() =>
            {
                ...
                catch (Exception e) { Log.LogMessage(LogLevel.Error, LogType.Generic, $"[Sts2Unlimited] Config error: {e.Message}\n{e.StackTrace}"); }
            }), (uint)GodotObject.ConnectFlags.OneShot);
        }
        catch (Exception e) { Log.LogMessage(LogLevel.Error, LogType.Generic, $"[Sts2Unlimited] Injection error: {e.Message}\n{e.StackTrace}"); }
    }
    ...
    private static void ConfigureMaxPlayersSlider(Node slider, Node sliderRow, int playerCount)
    {
        ...
        if (nameLabel != null)
            ...
            Log.LogMessage(LogLevel.Warn, LogType.Generic, "[Sts2Unlimited] 'Label' not found in sliderRow.");

        // ── 2. NSlider (Range) ───────────────────────────────────────────────
        var nslider = slider?.GetNodeOrNull("Slider") as Godot.Range;
        if (nslider == null) { Log.LogMessage(LogLevel.Warn, LogType.Generic, "[Sts2Unlimited] 'Slider' child not found."); return; }
        ...
        Log.LogMessage(LogLevel.Info, LogType.Generic,
            $"[Sts2Unlimited] Max Players slider configured: range [{PLAYER_MIN},{PLAYER_MAX}], current={playerCount}");
    }
    ...
    public static void SaveMaxPlayers(int value)
    {
        ...
        catch (Exception e) { Log.LogMessage(LogLevel.Error, LogType.Generic, $"[Sts2Unlimited] Failed to save settings: {e.Message}"); }
    }
}
